...
export default function Calendar() {
  ...
  useEffect(() => {
    Axios.get(`http://localhost:3001/IfValide/${userInfo.id}/${prId}`).then((response) => {
  ...
  const addwork = () => {
    ...
    if(projectsList.length !==0 ){
      ...
      if(valideData.length !==0){
        Axios.post("http://localhost:3001/updateProjectByCol", data)
        .then(res => {
      ...
      Axios.post("http://localhost:3001/createProjectByCol", data)
      .then(res => {
        console.log(res.data)
       //   history.push("/home")
      })
